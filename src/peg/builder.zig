...
const rule_dispatch = std.StaticStringMap(BuilderFn).initComptime(.{
    ...
    .{ "self_type", types_impl.buildSelfType },
...
test "builder - @this type parses without crash" {
    const alloc = std.testing.allocator;
    const peg = @import("../peg.zig");
    ...
    const s = result.node.program.top_level[0];
    try std.testing.expect(s.* == .struct_decl);
    // Field type should be type_named "@this"
    const field = s.struct_decl.members[0];
    try std.testing.expect(field.* == .field_decl);
    try std.testing.expect(field.field_decl.type_annotation.* == .type_named);
    try std.testing.expectEqualStrings("@this", field.field_decl.type_annotation.type_named);
}
