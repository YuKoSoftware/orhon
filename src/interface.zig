...
fn formatExprSimple(node: *parser.Node, buf: *std.ArrayListUnmanaged(u8), alloc: std.mem.Allocator) anyerror!void {
    switch (node.*) {
        ...
        .version_literal => |v| {
            ...
            try buf.appendSlice(alloc, ", ");
            try buf.appendSlice(alloc, v[1]);
            try buf.appendSlice(alloc, ", ");
            try buf.appendSlice(alloc, v[2]);
            try buf.append(alloc, ')');
        },
        .call_expr     => |c| {
...
test "emitInterfaceDecl - pub enum with variants" {
    const alloc = std.testing.allocator;
    var arena = std.heap.ArenaAllocator.init(alloc);
    defer arena.deinit();
    const a = arena.allocator();
