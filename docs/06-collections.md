...
| Type | Methods |
|------|---------|
| `List(T)` | `new()`, `withAlloc(alloc)`, `add(item)`, `get(index)`, `set(index, item)`, `remove(index)`, `pop()`, `len()`, `items()`, `free()` |
| `Map(K, V)` | `new()`, `withAlloc(alloc)`, `put(key, value)`, `get(key)`, `has(key)`, `remove(key)`, `keys()`, `values()`, `entries()`, `len()`, `free()` |
| `Set(T)` | `new()`, `withAlloc(alloc)`, `add(item)`, `has(item)`, `remove(item)`, `items()`, `len()`, `free()` |

`.new()` uses the default SMP allocator. `.withAlloc(alloc)` takes a custom allocator — see [[09-memory#Allocators]] for the three allocation modes.
