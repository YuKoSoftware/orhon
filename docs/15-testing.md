...
The built-in `@assert` is a compiler function. Inside a test block it lowers to
`std.testing.expect`, which reports a proper test failure. Outside a test block it
lowers to `std.debug.assert`, which panics on failure. Optionally takes a message:

```
@assert(x > 0)                        // test block: expect; elsewhere: assert
@assert(x > 0, "x must be positive")  // always: if (!(x > 0)) @panic("x must be positive")
```
