...
break, continue, true, false, extern
```
...
### Type annotation
Explicit when ambiguous, optional when unambiguous. See Section 7 for full rules. Numeric literals resolve to `main.bitsize` default or require an explicit type.
...
### Tuples
Two distinct kinds — no mixing allowed. Both must be defined as named types with `const`. Nominal typing — two tuples with identical structure but different names are different types.

**Anonymous tuple — positional, accessed by index:**
```
const RGB = (u8, u8, u8)

var color: RGB = RGB(255, 128, 0)
color[0]    // red
color[1]    // green
color[2]    // blue
```
...
p.x    // field access still available
p.y
```

### Tuple destructuring
Any tuple can be destructured into individual variables. Left side holds variable names in parentheses, right side is anything that returns a tuple. Variable count must match field count — hard compiler error otherwise.
```
...
// anonymous tuple destructuring — positional
const RGB = (u8, u8, u8)
func getColor() RGB { }
...
### Tuple math
Math operators (`+`, `-`, `*`, `/`, `%`) work on tuples with all-numeric fields. No operator overloading — these are built-in compiler rules.
...
### Error
`Error` is a proper storable type. Can be created inline or stored as a named constant for reuse. Errors are compared by identity — two errors with the same message string are not equal unless they are the same constant.

```
// inline
Error("division by zero")

// stored as reusable constant
const ErrDivByZero: Error = Error("ERROR: division by zero")
const ErrNotFound: Error = Error("ERROR: not found")
const ErrOverflow: Error = Error("ERROR: stack overflow")
...
// identity comparison
result.Error == ErrDivByZero    // true only if exact same constant
result.Error == ErrNotFound     // false

// access message
result.Error.message            // "ERROR: division by zero"
```

### Overflow helpers
See Section 15 — Integer Overflow for full documentation.
...
Functions that can fail return a union of `Error` and the success type. No exceptions, no monads — just an explicit union and a type check. If the programmer handles the error themselves, no propagation occurs. If the error is not handled before scope exit, the compiler propagates it automatically with a full trace.

Use `std.error` for common errors to avoid duplicate work. Define custom errors as named constants for reusability and identity comparison.

**Note:** `@type(result) == Error` uses `@type` which returns the actual type — this is a type comparison, not a string comparison. Use `@typename(result)` only when you need the type name as a string for display or logging.
...
// External libraries (C, system) — use a Zig bridge file, see Section 33
...
std.error         // common reusable errors organized by category — general, memory, IO, network, parsing, encoding
...
All errors in `std.error` use identity comparison. Organized by category:
...
## 33. Zig Bridge — `extern func` and paired `.zig` files
...
// --- TUPLES ---
const MinMax = (min: i32, max: i32)
const RGB = (u8, u8, u8)    // anonymous tuple
