...
@typeOf(x)               // returns the compile-time type of x — storable in a const type
...
@splitAt(slice, n)       // splits a slice at index n — must be used with destructuring (see [[06-collections]])
```
...
Returns a compile-time slice of all field names on a struct. Primary use: iterating fields in `compt func` for auto-derive patterns.
```
compt derive_debug() str {
    for(@fieldNames(Point)) |name| {
...
### Version literal
A plain literal of three integers following semantic versioning — major, minor, patch.
String versions are never allowed — hard compiler error. Used in `#version` metadata:
```
#version = (1, 0, 0)
```
