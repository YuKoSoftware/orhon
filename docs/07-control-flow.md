...
for(0..10) |i| { }                          // range: i = 0, 1, ..., 9 (exclusive end)
...
Range bounds in `for` are exclusive at the end — `0..10` iterates `i` = 0 through 9.

Note: `match` range patterns use inclusive bounds — `(1..10)` matches 1 through 10. The two contexts differ intentionally.
...
`match` is the way to safely branch on enum variants and union types. Must be exhaustive — compiler error if any variant is unhandled. `else` is the catch-all arm and must be last.
...
Arms are type names. Inside each arm body, the original variable has the unwrapped type — no dot syntax or binding syntax needed.

```
// (Error | T) — result is i32 in the value arm, Error (str) in the error arm
match result {
    Error => { console.println(result) }
    i32   => { return result }
}

// (null | T) — user is User in the value arm
match user {
    null => { return "not found" }
    User => { return user.name }
}

// (null | Error | T) — all three arms required (or use else)
match value {
    null  => { return 0 }
    Error => { return 0 }
    i32   => { return value }
...
When `if(x is T)` checks a union variable, `x` is automatically narrowed to type `T` inside the if body. No `.value` or dot syntax needed — the variable IS the checked type.

```
// Error union — x is i32 in the body
const result: (Error | i32) = compute()
if(result is not Error) {
    return result           // result is i32 here
}

// Null union — x is i32 in the body
const val: (null | i32) = find(5)
if(val is not null) {
    return val              // val is i32 here
}

// Early exit — narrowed after the if
const v: (null | Error | i32) = get()
if(v is null) { return 0 }
if(v is Error) { return 0 }
return v                    // v is i32 here (both null and error eliminated)

// Arbitrary union — x is i32 in the body
const data: (i32 | str) = 42
if(data is i32) {
    return data             // data is i32 here
}
```

Narrowing works cumulatively through elif chains and early-exit patterns. The `.value` syntax still works as an explicit alternative.

**Restrictions:** `is` can only be used in `if` and `elif` conditions. It cannot appear in `while`, assignments, or general expressions. For type checks outside if/elif, use `@typeOf(x) == T`. Compound `is` with `and`/`or` is not supported — use nested if statements instead.
