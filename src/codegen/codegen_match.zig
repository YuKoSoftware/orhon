...
pub fn generatePtrCoercion(cg: *CodeGen, kind: []const u8, type_node: *parser.Node, value: *parser.Node) anyerror!void {
    if (std.mem.eql(u8, kind, builtins.BT.PTR)) {
        ...
        const zig_type = try cg.typeToZig(type_node);
        if (value.* == .mut_borrow_expr or value.* == .const_borrow_expr) {
            // RawPtr(T) + &x → @as([*]T, @ptrCast(&x))
            try cg.emitFmt("@as([*]{s}, @ptrCast(", .{zig_type});
            try cg.generateExpr(value);
            try cg.emit("))");
...
pub fn fillDefaultArgs(cg: *CodeGen, c: parser.CallExpr) anyerror!void {
...
pub fn generateCollectionExpr(cg: *CodeGen, c: parser.CollectionExpr) anyerror!void {
