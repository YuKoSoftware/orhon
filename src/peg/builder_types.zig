...
// Contains: buildNamedType, buildKeywordType, buildSelfType, buildScopedType,
//           buildScopedGenericType, buildGenericType, buildBorrowType,
//           buildRefType, buildParenType, buildTupleTypeLiteral, buildSliceType,
//           buildArrayType, buildFuncType
...
/// self_type <- '@' 'this'
pub fn buildSelfType(ctx: *BuildContext, _: *const CaptureNode) !*Node {
    return ctx.newNode(.{ .type_named = "@this" });
}
...
/// tuple_type_literal <- '{' _ IDENTIFIER ':' type ('=' expr)? (',' IDENTIFIER ':' type ('=' expr)?)* '}'
/// Produces a type_tuple_named node with optional field defaults.
pub fn buildTupleTypeLiteral(ctx: *BuildContext, cap: *const CaptureNode) !*Node {
    // Collect all type and expr children in order
    var type_children = std.ArrayListUnmanaged(*Node){};
    var expr_children = std.ArrayListUnmanaged(ExprChild){};
    for (cap.children) |*child| {
        if (child.rule) |r| {
            if (std.mem.eql(u8, r, "type")) {
                try type_children.append(ctx.alloc(), try builder.buildNode(ctx, child));
            } else if (std.mem.eql(u8, r, "expr")) {
                try expr_children.append(ctx.alloc(), .{
                    .node = try builder.buildNode(ctx, child),
    ...
    // Walk tokens to find IDENTIFIER ':' patterns and match with type children
    var fields = std.ArrayListUnmanaged(parser.NamedTypeField){};
    ...
    while (i + 1 < cap.end_pos and i + 1 < ctx.tokens.len) : (i += 1) {
        if (ctx.tokens[i].kind == .identifier and ctx.tokens[i + 1].kind == .colon) {
            if (type_idx < type_children.items.len) {
                // Check for default: find '=' token after this type, before next ',' or '}'
                var default_node: ?*Node = null;
                for (expr_children.items) |ec| {
                    // The default expr must start after the identifier position
                    // and before the next field's identifier
                    if (ec.start_pos > i) {
                        // Check there's an '=' between the type and this expr
                        var j = i + 2; // skip past identifier and colon
                        while (j < ec.start_pos and j < ctx.tokens.len) : (j += 1) {
                            if (ctx.tokens[j].kind == .eq) {
                                default_node = ec.node;
                                break;
                            }
                            if (ctx.tokens[j].kind == .comma) break;
                        }
                        break;
                    }
                }
                try fields.append(ctx.alloc(), .{
    ...
    return ctx.newNode(.{ .type_tuple_named = try fields.toOwnedSlice(ctx.alloc()) });
}

const ExprChild = struct {
    node: *Node,
    start_pos: usize,
};
