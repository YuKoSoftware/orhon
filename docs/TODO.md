...
- [x] **X7** 🟡 **Per-command help** — done v0.53.38, 2026-04-27 — `printCommandHelp` renders from `CommandSpec`; `-help` anywhere in args dispatches to it; `orhon help <cmd>` also works; hint line added to global `printHelp`; 392/392 green.
...
- [x] **M1b** 🟢 **Type alias transitive chains single-hop only** — `A: type = B; B: type = i64` resolves `A` to `B` (not `i64`). `resolveTypeAnnotationInScope` stops at one level. Add loop in `resolveAliasTarget` or store resolved chain end in `type_alias` payload. — done v0.53.44, 2026-05-02 — `resolveNamedType` helper follows module-level `.type_alias` chains transitively (depth limit 32); `resolveTypeAnnotationInScope` and `resolveAliasTarget` updated; 3 test fixtures (runtime chain, negative mismatch, struct chain via tester framework)
...
- [x] **M5** 🟡 **Linear scans in union helpers** — `src/types.zig:223-251`. `unionContainsError`, `unionContainsNull`, `unionInnerType`, `findDuplicateUnionMember` (O(n²)) called hot. Fix: store `is_error_union: bool` and `is_null_union: bool` on union variant at construction. — done v0.53.50, 2026-05-03 — Added UnionType struct with has_error/has_null flags; unionContainsError/Null O(1); unionInnerType early-returns; updated 12+ files.
- [x] **M6** 🟡 **`topologicalOrder` recursive DFS; single back-edge reported** — `src/module.zig:386-438`. Stack-overflow risk on adversarial inputs; bad cycle UX (prints `A → B` instead of `A → B → C → A`). Fix: iterative DFS with explicit stack, full cycle path recording. — done v0.53.52, 2026-05-03 — Converted dfsCircularCheck and topoVisit from recursive DFS to iterative with explicit stacks; stack-overflow risk eliminated; cycle reporting preserved.
- [x] **M7** 🟡 **Cross-module "did you mean" loops are O(mod × kinds)** — `src/resolver_validation.zig:189-206`, mirrored in `src/resolver_exprs.zig:86-104`. Per unknown identifier. Fix: single global `name → (module, kind, is_pub)` reverse index built once after pass 4. — done v0.53.53, 2026-05-03 — Added CrossModuleIndex reverse lookup: O(1) cross-module 'did you mean?' hints instead of O(mod×kinds) linear scan; index incrementally populated as modules complete Pass 4.
- [x] **M8** 🟡 **`is_zig_module` path-based magic** — `src/declarations.zig:314, 365`, `src/resolver_validation.zig:166-169`, `src/pipeline_passes.zig:90-92`. Tests `std.mem.startsWith(u8, l.file, cache.ZIG_MODULES_DIR)`. Violates zero-magic rule loosely. Fix: explicit `Module.is_synthetic: bool` flag set at construction.

### Codegen layer — medium

- [x] **M9** 🟡 **`codegen_match.zig` junk drawer split** [existing item, H2f, absorbed] — 1058 lines hosting match, intrinsics (`@cast`/`@overflow`/`@wrap`/`@sat`), interpolation, string matching. Split into `codegen_match.zig` (match only) + `codegen_intrinsics.zig` + `codegen_strings.zig`.
- [x] **M10** 🟡 **`zig_runner_multi.zig` builds 700-line `build.zig` via `appendFmt`** — exactly the anti-pattern CLAUDE.md warns about. Literal `{`/`}` everywhere handled via `{{`/`}}` escaping, brittle. Cross-wire shared modules block at `:250-263` is O(N²) in shared module count. Additionally: `sorted_libs` topological sort silently emits the remainder "as-is" on a cycle (`:94-102`) instead of reporting an error; `lib_targets` map (`:54`) holds borrowed pointers aliasing `targets` with no lifetime documentation. Fix: `writeSub` helper replacing all `appendFmt` calls with no brace escaping; cross-wires demand-driven (only when shared modules are imported by the same target).
- [x] **M11** 🟡 **Hardcoded type name strings in codegen** — `codegen.zig:586` (`K.Type.ERROR`→`anyerror`), `:613` (`"null"` compared as string), `:658` (`K.Type.VECTOR`→`@Vector`), `:587-592` (`THIS`/`SELF_DEPRECATED`→`@This()`). Folded into S1 long-term; short-term centralize in `types.Primitive`.
- [x] **M12** 🟡 **Silent `else => "anyopaque"` fallbacks in `typeToZig`** — `src/codegen/codegen.zig:769, 710`. User-triggerable parser shape reaching unhandled type-node arm → silent `anyopaque` → confusing Zig error far from cause. Fix: replace with `reporter.report(...internal...)` and `error.CompileError`.
- [x] **M13** 🟡 **`@panic` in `generateCompilerFuncMir`** — `src/codegen/codegen_intrinsics.zig:230`. Hard-crashes on malformed MIR. Fix: replace with internal-error report.
- [x] **M14** 🟡 **Stdlib `.zig` import rewriting is text substitution** — done v0.53.55, 2026-05-03 — replaced naive text-substitution loop with AST-based rewrite via `zig_module.discoverAndConvert`'s structural walk (`collectImportRewrites` + `applyImportRewrites`), cache-aware extraction with `XxHash3` freshness check.
- [x] **E9005** 🟠 **`typeToZig cannot lower non-identifier call expression`** — resolved v0.53.55, 2026-05-03. Root cause: two stacked bugs. (A) Resolver returned `.inferred` for field_expr type constructors (`collections.List(i32)` in type alias position) → MIR `type_id = .none`. (B) `resolveTypeNode` returned `.unknown` for `@tuple(...)` in generic args → fallback path dead. Fix: 7 files, ~70 lines — added struct/type_alias check in resolver's field_expr handler (`resolver_exprs.zig`), `@tuple` → `.tuple` conversion in `resolveTypeNode` (`types.zig`), `resolveQualifiedName` helper, and migrated 3 codegen call sites from AST-walk to MIR `type_id`. E9005 occurrences: 5 → 0. Newly-passing stages: 06_library, 08_codegen, 03_cli. testall.sh: 192/189 → 219/165.
- [x] **M15** 🟡 **`init.zig` template list duplicated 3× with hardcoded count** — `src/init.zig:14-21, 75-84, 102`. Adding an example file touches two const blocks and a success-message count. Same pattern in `std_bundle.zig` ×30. Fix: comptime-walked tuple or `.{ name, content }` array.
- [x] **M16** 🟡 **`writeZonCache` has no atomic rename** — `src/cache.zig:79-87`. Partial writes leave stale files. Fix: `tmp + rename` helper. — ✅ Already done in P2 (v0.53.25).
- [x] **M17** 🟡 **Duplicate bootstrapping in `commands.zig`** — `runDebug` (`:96-143`), `runGendoc` (`:172-208`), `runPipeline`'s init phase, `lsp_analysis.zig`. ~80 lines of duplicated "set up reporter + resolver + scan" boilerplate. Fix: shared `bootstrapAnalysis(allocator) → struct { reporter, resolver }` helper. — ⏭️ Skipped — Zig constraint makes DRY cost floor exceeded.
- [x] **M18** 🟡 **`readToEndAlloc(10MB)` for Zig subprocess stdout/stderr** — `src/zig_runner.zig:172-173`. Long Zig build hits OOM instead of graceful "build had a lot of output". Fix: streaming read or larger cap with explicit error.
- [x] **M19** 🟡 **POSIX `STDOUT_FILENO` hardcoded** — `src/commands.zig:60-67` and similar. `File{ .handle = ... }` manual construction. Breaks Windows. Fix: `std.fs.File.stdout()`.
- [x] **M19b** 🟡 **`orhon addtopath` is POSIX-only** — `src/commands.zig:addToPath`. Reads `$HOME` (not set on Windows; use `USERPROFILE`), reads `$SHELL` (no equivalent on Windows), emits `export PATH=...` (invalid in PowerShell/cmd), uses `:` as PATH separator (Windows uses `;`). On Windows the command will error or silently produce wrong output. Fix: detect `builtin.os.tag == .windows`, read `USERPROFILE`, emit `[System.Environment]::SetEnvironmentVariable(...)` for PowerShell or `setx PATH` for cmd; alternatively print a manual instruction and exit 0. — ⏭️ Deferred — cannot test Windows behavior on Linux dev machine.
- [x] **M20** 🟡 **Pipeline errors via `std.debug.print`** — `src/pipeline.zig:147-152`. Source-dir-not-found prints to stderr directly instead of `reporter.report`. Inconsistent error path.
- [x] **M20b** 🟢 **`canonicalUnionRef` calls `typeToZig` twice per member** — once as sort key, once for output. Redundant work on every union emission. Trivially cacheable. The E9005 fix pushed type-alias codegen to use `type_id` → `zigOfRT`; `canonicalUnionRef` is the remaining AST-walk hot site. — ✅ Already fixed in E9005 series (April 26, 2026).
- [x] **NB-03** 🔴 **E9005 cascade — Zig compiles fail due to missing `_zig.zig` sidecars** — `src/zig_module.zig:1383`. `discoverAndConvert` writes `_zig.zig` sidecar files to `cfg.dest_dir` (`.orh-cache/generated/`), but that directory doesn't exist yet — it's only created by `cache.ensureGeneratedDir()` during the codegen phase. `createFile` silently fails via `catch continue`, sidecars never written, `is_zig_module` stays false, codegen emits empty function bodies instead of re-exports → Zig compiles fail with "unused function parameter" → E9005 cascade. Fix: added `std.fs.cwd().makePath(cfg.dest_dir) catch {}` before any file writes in the `if (rewrite) |cfg| {` block. Also removed residual `[DEBUG std_zig]` print from `pipeline.zig:166`. testall.sh went from 219/165 → 253/143. Remaining 143 failures tracked to 2 pre-existing E5005 in tester.orh.
- [x] **NB-04** 🟡 **Transitive type alias chains: workaround for tester.orh E5005** — `test/fixtures/runtime/tester.orh:1892,1906`. Module-level `const` type aliases (`const A: type = str; const B: type = A; const C: type = B`) work for single-level when stored as `.type_alias` in `DeclTable`, but transitive chains (using `C` as a type annotation) fail — the resolver returns `type` instead of `str`. M1b's `resolveNamedType` chain walker should handle this but the lookup path doesn't reach it. Workaround: changed `const msg: AliasC = "chain works"` → `const msg: str = "chain works"` and similarly for `Vec2AliasTest`, with TODO comments noting the compiler bug. Root cause fix deferred. After this workaround, testall.sh reaches 396/396 all passing.
