...
pub fn formatProject(allocator: std.mem.Allocator, source_dir: []const u8) !void {
    ...
    try formatDirRecursive(allocator, &dir, source_dir);
}

fn formatDirRecursive(allocator: std.mem.Allocator, dir: *std.fs.Dir, dir_path: []const u8) !void {
    var it = dir.iterate();
...
fn formatFile(allocator: std.mem.Allocator, path: []const u8) !void {
    ...
    const formatted = try format(allocator, content);
