// codegen_decls.zig — Declaration generators for the Orhon code generator
// Contains: func, struct, enum, bitfield, var/const, compt, and test declaration codegen.
...
pub fn generateFunc(cg: *CodeGen, node: *parser.Node, f: parser.FuncDecl) anyerror!void {
    ...
    if (f.is_pub or std.mem.eql(u8, f.name, "main")) try cg.emit("pub ");
    ...
    for (f.params, 0..) |param, i| {
        ...
        if (param.* == .param) {
            ...
            if (is_type_param) {
                ...
                try cg.emitFmt("{s}: anytype", .{param.param.name});
            } else {
                try cg.emitFmt("{s}: {s}", .{
                    param.param.name,
                    try cg.typeToZig(param.param.type_annotation),
                });
    ...
    if (return_is_any) {
        if (first_any_param) |pname| {
            ...
            try cg.emit("anyopaque"); // fallback: no any param found
        }
    } else {
        try cg.emit(try cg.typeToZig(f.return_type));
    }
    try cg.emit(" ");

    // Body — MIR block path
    const func_mir = cg.current_func_mir orelse return error.CompileError;
    try cg.generateBlockMir(func_mir.body());
    try cg.emit("\n");
}
...
pub fn generateThreadFunc(cg: *CodeGen, node: *parser.Node, f: parser.FuncDecl) anyerror!void {
    ...
    const inner_zig = try cg.typeToZig(inner_type);
    const handle_zig = try cg.typeToZig(f.return_type);
    ...
    // ── Body function: fn _name_body(params) T { ... } ──
    {
        ...
        try cg.emitFmt("fn _{s}_body(", .{f.name});
        for (f.params, 0..) |param, i| {
            if (i > 0) try cg.emit(", ");
            if (param.* == .param) {
                try cg.emitFmt("{s}: {s}", .{
    ...
    const is_void = std.mem.eql(u8, inner_zig, "void");
    if (!is_void) try cg.emit("_s.result = ");
    try cg.emitFmt("_{s}_body(", .{f.name});
    for (f.params, 0..) |param, i| {
        if (i > 0) try cg.emit(", ");
        if (param.* == .param) {
            try cg.emitFmt("_{s}", .{param.param.name});
        }
    }
    try cg.emit("); _s.completed.store(true, .release); } }.run, .{ _state");
    for (f.params) |param| {
        if (param.* == .param) {
            try cg.emitFmt(", {s}", .{param.param.name});
        }
    }
    try cg.emit(" }) catch |e| @panic(@errorName(e)), .state = _state };\n");
...
pub fn generateConst(cg: *CodeGen, node: *parser.Node, v: parser.VarDecl) anyerror!void {
    if (v.is_bridge) return cg.generateBridgeReExport(v.name, v.is_pub);
    return cg.generateDecl(node, v, "const");
}

pub fn generateVar(cg: *CodeGen, node: *parser.Node, v: parser.VarDecl) anyerror!void {
    if (v.is_bridge) return cg.generateBridgeReExport(v.name, v.is_pub);
    return cg.generateDecl(node, v, "var");
}

/// Shared codegen for var and const declarations
pub fn generateDecl(cg: *CodeGen, node: *parser.Node, v: parser.VarDecl, decl_keyword: []const u8) anyerror!void {
    if (v.is_pub) try cg.emit("pub ");
    try cg.emitFmt("{s} {s}", .{ decl_keyword, v.name });
    const tc = cg.getTypeClass(node);
...
pub fn generateStmtDecl(cg: *CodeGen, node: *parser.Node, v: parser.VarDecl, decl_keyword: []const u8) anyerror!void {
    const tc = cg.getTypeClass(node);
    try cg.emitFmt("{s} {s}", .{ decl_keyword, v.name });
    if (v.type_annotation) |t| try cg.emitFmt(": {s}", .{try cg.typeToZig(t)});
    try cg.emit(" = ");
    if (tc == .arbitrary_union) {
        try cg.generateArbitraryUnionWrappedExpr(v.value, cg.getUnionMembers(node));
    } else {
...
pub fn generateCompt(cg: *CodeGen, v: parser.VarDecl) anyerror!void {
    // Top-level const is already comptime in Zig, so just emit const.
    if (v.is_pub) try cg.emit("pub ");
    try cg.emitFmt("const {s}: {s} = ", .{
        v.name,
        try cg.typeToZig(v.type_annotation orelse return),
    });
