...
pub fn checkExpr(self: *OwnershipChecker, node: *parser.Node, scope: *OwnershipScope, is_borrow: bool) anyerror!void {
    switch (node.*) {
        ...
        .call_expr => |c| {
            // Method call: obj.method(args) — check if self is consumed (by-value)
            if (c.callee.* == .field_expr) {
                const fe = c.callee.field_expr;
                if (fe.object.* == .identifier) {
                    const obj_name = fe.object.identifier;
                    if (self.lookupStructMethod(scope, obj_name, fe.field)) |sig| {
                        if (sig.is_instance) {
                            const self_node = sig.param_nodes[0];
                            const is_by_value = self_node.* == .param and self_node.param.type_annotation.* != .type_ptr;

                            // Check args first (before marking receiver moved)
                            for (c.args) |arg| {
                                const arg_is_borrow = arg.* == .mut_borrow_expr or arg.* == .const_borrow_expr;
                                try checkExpr(self, arg, scope, arg_is_borrow);
                            }

                            // Check the receiver for use-after-move
                            try checkExpr(self, fe.object, scope, true);

                            // By-value self consumes the receiver — mark moved (non-primitive only)
                            if (is_by_value) {
                                if (scope.getState(obj_name)) |state| {
                                    if (!state.is_primitive and !state.is_const) {
                                        _ = scope.setState(obj_name, .moved);
                                    }
                                }
                            }
                            return;
